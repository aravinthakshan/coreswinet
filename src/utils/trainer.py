    ...
    for epoch in range(epochs):
        ...
        with tqdm(train_loader, desc=f"Epoch {epoch+1}/{epochs} - Training Progress") as loader:
            for itr, batch_data in enumerate(loader):
                ...
                # Always use noisy input
                output, f1, f2 = model(noise, noise)
                
                # Calculate losses based on training phase
                mse_loss = mse_criterion(output, clean)
                
                if not threshold_reached:
                    # Before threshold: MSE + Contrastive
                    contrastive_loss = contrastive_loss_fn(f1, f2)
                    loss = mse_loss + 0.01 * contrastive_loss
                else:
                    # After threshold: Only MSE
                    loss = mse_loss
                ...
                loader.set_postfix(
                    ...
                    psnr=psnr_train_itr, 
                    ssim=ssim_train_itr,
                    mode="Combined" if not threshold_reached else "MSE only"
                )
            ...
            print(f'Training Mode: {"Combined Loss" if not threshold_reached else "MSE Loss Only"}')
        ...
        if not threshold_reached and max_psnr > psnr_threshold:
