...
from src.utils.model.archs.arch_util_new import LayerNorm2d
from src.utils.model.archs.local_arch import Local_Base
...
class CascadedGazeNetBigger(nn.Module):
    ...
    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[], GCE_CONVS_nums=[],contrastive=True):
        ...
        self.middle_blks2=nn.ModuleList()
    ...
    def forward(self, inp,gt,bypass):
        ...
        for encoder, down in zip(self.encoders, self.downs):
            ...
            encs.append(x)
            x = down(x)
        for encoder,down in zip(self.encoders2,self.downs2):
            x_gt=encoder(x_gt)
            encs2.append(x_gt)
            x_gt=down(x_gt)

        x = self.middle_blks(x)
        x_gt=self.middle_blks2(x_gt)
        print(x.shape)
        if self.bypass!=True:
            for decoder, up, enc_skip,enc_skip2 in zip(self.decoders, self.ups, encs[::-1],encs2[::-1]):
                x = up(x)
                x = x + ((enc_skip+enc_skip2)/2)
                x = decoder(x)
        else:    
...
class CascadedGazeNetBiggerLocal(Local_Base, CascadedGazeNetBigger):
    def __init__(self, *args, train_size=(1, 3, 256, 256), fast_imp=False, **kwargs):
        Local_Base.__init__(self)
        CascadedGazeNetBigger.__init__(self, *args, **kwargs)
...
if __name__ == '__main__':
    ...
    net = CascadedGazeNetBigger(img_channel=img_channel,width=width, middle_blk_num=middle_blk_num,
