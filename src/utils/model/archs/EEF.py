...
class DF(nn.Module):
    def __init__(self, channel_num):
        super().__init__()
        # Ensure channel_num is divisible by 3 for the output channels
        self.out_channels = (channel_num // 3) * 3
        self.groups = max(1, self.out_channels // 3)
        
        self.C0 = nn.Sequential(
            nn.Conv2d(channel_num, self.out_channels//3, groups=self.groups, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(inplace=True))
        self.C1 = nn.Sequential(
            nn.Conv2d(channel_num, self.out_channels//3, groups=self.groups, kernel_size=3, stride=1, padding=2, dilation=2),
            nn.LeakyReLU(inplace=True))
        self.C2 = nn.Sequential(
            nn.Conv2d(channel_num, self.out_channels//3, groups=self.groups, kernel_size=3, stride=1, padding=3, dilation=3),
            nn.LeakyReLU(inplace=True))
        self.R = nn.GELU()
    ...
    def forward(self, x):
        ...
        return torch.cat((l, m, h), dim=1)

class EFF(nn.Module):
    def __init__(self, dim=32, out_dim=128):
        super().__init__()
        self.linear1 = nn.Sequential(nn.Linear(dim, dim), nn.GELU())
        self.DF = DF(dim)
