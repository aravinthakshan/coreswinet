...
class Model(nn.Module):
    def __init__(self, in_channels=3):
        ...
        # Initialize the base UNet model
        self.unet = smp.Unet(
    ...
    def forward(self, x):
        # Get features from original encoder
        features = self.encoder(x)
        
        # Apply Squeeze-and-Excitation to bottleneck
        bottleneck_feat = self.bottleneck_attention(features[-1])
        ...
        # Use decoder with processed features
        decoder_output = self.unet.decoder(*processed_features, bottleneck_feat)
